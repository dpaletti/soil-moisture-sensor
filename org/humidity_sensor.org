...
** PCB development
*** Block diagram
#+begin_src mermaid :file block_diagram.png :mermaid-config-file "./resources/mermaid_config.json"
graph TD
...
subgraph HUMIDITY["SOIL PERMITTIVITY"]
         ...
         LINE --> |feeds back| OSCILLATOR
         MEASURED_FREQUENCY --> |goes through| DIVIDER["FREQUENCY DIVIDER"]
end

USB-C --> |feeds 5V| VOLTAGE-REGULATOR
VOLTAGE-REGULATOR --> |recharges at 1.5V| BATTERY
BATTERY -->|powers| MICROCONTROLLER
BATTERY -->|powers| HUMIDITY
BATTERY -->|powers| TEMPERATURE

TEMPERATURE -->|sends signal| MICROCONTROLLER
HUMIDITY -->|sends signal| MICROCONTROLLER
#+end_src
#+RESULTS:
